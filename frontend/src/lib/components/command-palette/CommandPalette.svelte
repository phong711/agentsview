  ...
  <div class="palette">
    <div class="palette-input-wrap">
      ...
      <input
        ...
        type="text"
        class="palette-input"
        placeholder={t(m.command_palette_placeholder)}
        value={inputValue}
        oninput={handleInput}
      />
    ...
    <div class="palette-results">
      {#if showSearchResults}
        <div class="palette-sort">
          ...
          >{t(m.command_palette_relevance)}</button>
          <button
            class="sort-btn"
            class:active={searchStore.sort === "recency"}
            onmousedown={(e: MouseEvent) => e.preventDefault()}
            onclick={() => { searchStore.setSort("recency"); selectedIndex = 0; }}
          >{t(m.command_palette_recency)}</button>
        </div>
        {#if searchStore.isSearching}
          <div class="palette-empty">{t(m.command_palette_searching)}</div>
        {:else if searchStore.results.length === 0}
          <div class="palette-empty">{t(m.command_palette_no_results)}</div>
        {:else}
          {#each searchStore.results as result, i}
            ...
            >
              ...
              <span
                class="item-id"
                title={t(m.command_palette_copy_session_id)}
                onclick={(e) => {
                  e.stopPropagation();
        ...
        {/if}
      {:else}
        <div class="palette-section-label">{t(m.command_palette_recent_sessions)}</div>
